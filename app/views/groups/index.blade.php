...
<div class="row">
  <div class="col-md-10 col-md-offset-1">
	<div class="table-responsive">
		<table class="table table-striped table-hover">
			...
			@foreach ($groups as $group)
				<tr>
					...
					<td>
						<button class="btn btn-primary" onClick="location.href='{{ action('GroupController@edit', array($group->id)) }}'">Edit</button>
					 	<button class="btn btn-danger action_confirm {{ ($group->id == 2) ? 'disabled' : '' }}" type="button" data-method="delete" href="{{ URL::to('groups') }}/{{ $group->id }}">Delete</button>
					 </td>
				</tr>	
			@endforeach
			</tbody>
	 ...
	 <button class="btn btn-success" onClick="location.href='{{ URL::to('groups/create') }}'">New Group</button>
   </div>
</div>
<!--  
